# 3.1.3. Deploy applications to Azure Kubernetes Services

Completed
100 XP
10 minutes

## After writing your Deployment and Service manifests, deploying to AKS is straightforward

- Use kubectl apply to send manifests to your cluster
- Kubernetes then reconciles current state with your desired state by pulling images, launching Pods, creating Services, and assigning public IPs if needed

### Imagine writing your Deployment and Service manifests and saving them as deployment.yaml and service.yaml

- You run kubectl apply, and moments later, your AI inference API is running on AKS with a public IP address

## Deploying with kubectl apply

- The kubectl apply command is your primary tool for deploying to AKS
- It reads your YAML manifests and tells cluster to create or update resources:
...
### Kubernetes processes of manifests and:

- Pulls your container image from registry
- Schedules Pods on cluster nodes
- Starts your containers
- Creates Service and assigns networking
...
#### READY: How many replicas are ready

- "2/2" means both replicas are running

#### STATUS: Pod state

- "Running" is what you want
- "Pending" means Pod is still starting (pulling image, waiting for resources)
- "CrashLoopBackOff" means your app is crashing—read logs to diagnose
...
### The EXTERNAL-IP (for LoadBalancer) or node IPs (for NodePort) is what clients use to reach your service

- If EXTERNAL-IP is <pending>, load balancer is still being provisioned—wait a moment and check again

## Checking Connectivity

- Verify your application is actually responding to requests:

#### For ClusterIP services (internal only): Run a test Pod and call service DNS name:
...
# Inside the debug Pod, test service
wget http://inference-api-service:80
```

#### For LoadBalancer services (external): Use public IP to test from your machine:
...
### If service is reachable, you get a response from your application

- If it times out or refuses connection, troubleshoot using scenarios below
...
#### Cause: Kubernetes can't pull your container image from registry
...
- Verify image path, tag, and registry are correct in your manifest
- Confirm image was built and pushed to your registry: az acr repository list --name myregistry
- Ensure your AKS cluster has credentials to pull from your registry (usually automatic with Azure Container Registry)
- Try pulling image manually to test: docker pull myregistry.azurecr.io/inference-api:v1.0
...
- Test your container image locally to ensure application starts: docker run myregistry.azurecr.io/inference-api:v1.0
...
- Add more nodes to cluster if capacity is exhausted: az aks scale --resource-group mygroup --name myaks --node-count 3
...
#### Cause: No Pods are matching Service's selector—traffic has nowhere to go
...
# If Endpoints is empty, selector isn't matching Pods
```

#### Solutions:

- Verify Pod labels match Service selector: compare kubectl get pods --show-labels with Service's selector
- Update either Pod labels or Service selector so they match
- Ensure Deployment's Pod template has correct label matching Service selector

## Additional Resources

### kubectl Command Reference - Complete kubectl command documentation
### Kubernetes Troubleshooting Guide - Debugging applications in Kubernetes
### AKS Best Practices - Production readiness practices for AKS
